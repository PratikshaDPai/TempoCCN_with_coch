...
def demo_human_cochleagram(signal=None, sr=None, n=None):
  ...
  ##coc=np.save(coch)
  file= open("coch.txt", "w")
  savetxt('coch.csv', coch, delimiter=',')
  ##file.write(coch)
  plt.subplot(324)
  ...
  cochd_poly = demo_human_cochleagram_helper(signal, sr, n, downsample=9600)
...
"""
  ...
  if signal is None:
    ...
    assert sr is not None
    assert n is not None
    low_lim = 20  # this is the default for cochleagram.human_cochleagram
    hi_lim = 5000 #20000  # this is the default for cochleagram.human_cochleagram

  # generate a cochleagram from the signal  sample_factor = 2  # this is the default for cochleagram.human_cochleagram
...
def make_harmonic_stack(f0=100, n_harm=40, dur=0.25001, sr=12000, low_lim=20, hi_lim=5000, n=None):
...
def main(ignore_playback_warning=False, mode='rand_sound'):
  ...
  DEMO_PATH = '/media/sf_acm_mirum_tempo' #join(dirname(realpath(__file__)), 'demo_stim')
  if mode == 'rand_sound':
